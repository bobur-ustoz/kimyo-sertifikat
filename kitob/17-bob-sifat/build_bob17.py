# -*- coding: utf-8 -*-
"""17-bob (Sifat reaksiyalari) — kitob dizaynidagi bob-PDF: pasport + A-variant + B-variant, har biri kaliti bilan."""
import json, html

data_A = json.load(open("mavzu_IV2A.json", encoding="utf-8"))
data_B = json.load(open("mavzu_IV2B.json", encoding="utf-8"))
ACCENT, DARK, TINT, ACCENT2 = "#ad1457", "#78103c", "#fdf2f6", "#f9a825"
...
def svg(curve, xlab="t", ylab="v"):
    ...
    return ('<svg width="118" height="86" viewBox="0 0 152 100">'
            ...
            f'{km}<path d="{p}" fill="none" stroke="#ad1457" stroke-width="2.4" '
            'stroke-linecap="round" stroke-linejoin="round"/></svg>')

# ---------- IV.2 figuralari (malina + amber palitrasi) ----------
I1, I2, ID, IP, IG = "#ad1457", "#f9a825", "#78103c", "#fdf2f6", "#f3d1de"

def fig_flame_panel():
    """Alanga ranglari paneli: 1-qizil, 2-sariq, 3-binafsha, 4-yashil."""
    flames = [("1", "#e53935", "Li/Sr"), ("2", "#fdd835", "Na"), ("3", "#8e24aa", "K"), ("4", "#43a047", "Ba/Cu")]
    H = [f'<svg width="270" height="128" viewBox="0 0 270 128">'
         f'<style>.lb{{font-size:8.4px;font-family:Georgia,serif;fill:{ID}}}</style>']
    for i, (num, col, lab) in enumerate(flames):
        x = 36 + i * 60
        H.append(f'<rect x="{x-4}" y="84" width="32" height="14" rx="3" fill="#78909c" stroke="#455a64" stroke-width="1.2"/>'
                 f'<rect x="{x+8}" y="70" width="8" height="16" fill="#90a4ae" stroke="#455a64" stroke-width="0.8"/>'
                 f'<path d="M{x+12},68 q-10,-16 0,-32 q10,16 0,32" fill="{col}" stroke="{ID}" stroke-width="0.8" opacity="0.9"/>'
                 f'<text x="{x+12}" y="112" text-anchor="middle" class="lb" font-weight="bold">{num}-alanga</text>')
    H.append(f'<text x="36" y="16" class="lb" font-weight="bold">alanga ranglari: metallning «imzosi»</text></svg>')
    return "".join(H)

def fig_precip_panel():
    """Cho'kmalar paneli: 1-oq, 2-ko'k, 3-qo'ng'ir, 4-qora."""
    tubes = [("1", "#fdfdf6", "oq"), ("2", "#3d78c9", "ko'k"), ("3", "#8d5524", "qo'ng'ir"), ("4", "#333333", "qora")]
    H = [f'<svg width="270" height="132" viewBox="0 0 270 132">'
         f'<style>.lb{{font-size:8.4px;font-family:Georgia,serif;fill:{ID}}}</style>']
    for i, (num, col, lab) in enumerate(tubes):
        x = 44 + i * 56
        H.append(f'<path d="M{x},18 v66 a10,10 0 0 0 20,0 v-66" fill="none" stroke="{ID}" stroke-width="1.6"/>'
                 f'<rect x="{x+2}" y="46" width="16" height="30" rx="2" fill="{I1}" opacity="0.08"/>'
                 f'<path d="M{x+2},72 a8,10 0 0 0 16,0 v-6 h-16 z" fill="{col}" stroke="{IG}" stroke-width="0.6"/>'
                 f'<text x="{x+10}" y="116" text-anchor="middle" class="lb" font-weight="bold">{num}</text>')
    H.append(f'<text x="40" y="12" class="lb" font-weight="bold">cho\'kmalar «rang lug\'ati»</text></svg>')
    return "".join(H)

def fig_titr_curve():
    """Cho'kma massasi vs qo'shilgan AgNO3 — rise_flat, sinish nuqtasi belgili."""
    return ('<svg width="260" height="146" viewBox="0 0 260 146">'
            f'<style>.lb{{font-size:8.4px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<rect x="36" y="6" width="216" height="120" rx="4" fill="{IP}" stroke="{IG}" stroke-width="1.1"/>'
            f'<line x1="36" y1="126" x2="252" y2="126" stroke="{ID}" stroke-width="1.4"/>'
            f'<line x1="36" y1="126" x2="36" y2="8" stroke="{ID}" stroke-width="1.4"/>'
            f'<path d="M38,124 L138,38 L246,38" fill="none" stroke="{I1}" stroke-width="2.4"/>'
            f'<circle cx="138" cy="38" r="3.4" fill="{I2}" stroke="{ID}" stroke-width="0.8"/>'
            f'<line x1="138" y1="38" x2="138" y2="126" stroke="{IG}" stroke-width="1" stroke-dasharray="3,3"/>'
            f'<text x="120" y="28" class="lb" font-weight="bold" fill="{I2}">sinish nuqtasi</text>'
            f'<text x="150" y="52" class="lb">m(AgCl) = 7,175 g</text>'
            f'<text x="6" y="16" class="lb">m(cho\'kma)</text>'
            '<text x="176" y="140" class="lb">V(AgNO₃)</text></svg>')

def fig_bar_agx():
    """0,1 mol AgCl/AgBr/AgI massalari — ustunlar."""
    data = [("AgCl", 14.35), ("AgBr", 18.8), ("AgI", 23.5)]
    mx = 26
    bars = ""
    for i, (lab, v) in enumerate(data):
        x = 62 + i * 62; h = v / mx * 106; y = 122 - h
        col = I1 if i != 2 else I2
        bars += (f'<rect x="{x}" y="{y:.0f}" width="36" height="{h:.0f}" rx="2" fill="{col}" opacity="0.88" '
                 ...
                 f'<text x="{x+18}" y="135" text-anchor="middle" class="lb">{lab}</text>')
    return ('<svg width="260" height="148" viewBox="0 0 260 148">'
            f'<style>.lb{{font-size:8.2px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<rect x="48" y="4" width="206" height="118" rx="4" fill="{IP}" stroke="{IG}" stroke-width="1.1"/>'
            + "".join(f'<line x1="50" y1="{122-g/26*106:.0f}" x2="252" y2="{122-g/26*106:.0f}" stroke="{IG}" stroke-width="0.9"/>'
                      f'<text x="34" y="{125-g/26*106:.0f}" class="lb">{g}</text>' for g in [10, 20])
            + bars +
            f'<line x1="48" y1="122" x2="254" y2="122" stroke="{ID}" stroke-width="1.5"/>'
            '<text x="4" y="14" class="lb">m (0,1 mol), g</text></svg>')

def fig_scheme38():
    """B O1-38: noma'lum kukun tekshiruv sxemasi."""
    return ('<svg width="286" height="88" viewBox="0 0 286 88">'
            f'<style>.lb{{font-size:8px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<rect x="4" y="26" width="66" height="34" rx="4" fill="{IP}" stroke="{I1}" stroke-width="1.6"/>'
            '<text x="37" y="42" text-anchor="middle" class="lb" font-weight="bold">oq kukun</text>'
            '<text x="37" y="54" text-anchor="middle" class="lb">5,3 g</text>'
            f'<line x1="70" y1="43" x2="92" y2="43" stroke="{I2}" stroke-width="2"/>'
            f'<polygon points="96,43 88,39 88,47" fill="{I2}"/>'
            f'<rect x="98" y="26" width="76" height="34" rx="4" fill="{IP}" stroke="{I1}" stroke-width="1.6"/>'
            '<text x="136" y="42" text-anchor="middle" class="lb" font-weight="bold">alanga: sariq</text>'
            '<text x="136" y="54" text-anchor="middle" class="lb">→ Na⁺</text>'
            f'<line x1="174" y1="43" x2="196" y2="43" stroke="{I2}" stroke-width="2"/>'
            f'<polygon points="200,43 192,39 192,47" fill="{I2}"/>'
            f'<rect x="202" y="26" width="80" height="34" rx="4" fill="{IP}" stroke="{I1}" stroke-width="1.6"/>'
            '<text x="242" y="42" text-anchor="middle" class="lb" font-weight="bold">+HCl: gaz!</text>'
            '<text x="242" y="54" text-anchor="middle" class="lb">CO₂ = ? L</text></svg>')

def fig_iodized():
    """Yodlangan tuz qadog'i."""
    return ('<svg width="220" height="118" viewBox="0 0 220 118">'
            f'<style>.lb{{font-size:8.6px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<rect x="46" y="26" width="56" height="70" rx="6" fill="{IP}" stroke="{ID}" stroke-width="1.6"/>'
            f'<rect x="52" y="38" width="44" height="26" rx="3" fill="#fff" stroke="{IG}" stroke-width="1"/>'
            '<text x="74" y="50" text-anchor="middle" class="lb" font-weight="bold">TUZ</text>'
            '<text x="74" y="60" text-anchor="middle" class="lb" font-size="6.4">yodlangan</text>'
            + "".join(f'<circle cx="{x}" cy="{y}" r="1.5" fill="#fff" stroke="{IG}" stroke-width="0.5"/>'
                      for x, y in [(60, 78), (70, 84), (80, 76), (88, 84)])
            + '<text x="114" y="42" class="lb" font-weight="bold">KIO₃ / KI</text>'
            '<text x="114" y="58" class="lb">qalqonsimon bez</text>'
            '<text x="114" y="70" class="lb">uchun yod manbai</text>'
            '<text x="44" y="112" class="lb" font-weight="bold">yodlangan osh tuzi</text></svg>')

def fig_mineral():
    """Mineral suv butilkasi va etiketka."""
    return ('<svg width="220" height="122" viewBox="0 0 220 122">'
            f'<style>.lb{{font-size:8.2px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<path d="M60,20 h16 v10 q10,6 10,18 v46 a8,8 0 0 1 -8,8 h-20 a8,8 0 0 1 -8,-8 v-46 q10,-12 10,-18 z" '
            f'fill="#dff0fa" stroke="{ID}" stroke-width="1.6"/>'
            f'<rect x="54" y="56" width="30" height="28" rx="2" fill="#fff" stroke="{IG}" stroke-width="1"/>'
            '<text x="69" y="66" text-anchor="middle" class="lb" font-size="5.8">Ca²⁺ 80</text>'
            '<text x="69" y="74" text-anchor="middle" class="lb" font-size="5.8">Mg²⁺ 24</text>'
            '<text x="69" y="82" text-anchor="middle" class="lb" font-size="5.8">SO₄²⁻ 30</text>'
            '<text x="104" y="46" class="lb" font-weight="bold">etiketka —</text>'
            '<text x="104" y="58" class="lb">tahlil natijasi</text>'
            '<text x="104" y="76" class="lb">(mg/L da ionlar)</text>'
            '<text x="46" y="116" class="lb" font-weight="bold">mineral suv tarkibi</text></svg>')

def fig_paints():
    """Rassom palitrasi va pigmentlar."""
    return ('<svg width="220" height="116" viewBox="0 0 220 116">'
            f'<style>.lb{{font-size:8.6px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<path d="M40,50 a34,26 0 1 0 68,10 q-6,-8 2,-14 a34,26 0 0 0 -70,4z" '
            f'fill="#e6cfa3" stroke="{ID}" stroke-width="1.6"/>'
            + "".join(f'<circle cx="{x}" cy="{y}" r="5" fill="{c}" stroke="{ID}" stroke-width="0.7"/>'
                      for x, y, c in [(56, 48, "#2e7d32"), (72, 42, "#1565c0"), (88, 48, "#c62828"), (64, 62, "#8d5524")])
            + '<text x="124" y="40" class="lb" font-weight="bold">malaxit — yashil</text>'
            '<text x="124" y="54" class="lb">azurit — ko\'k</text>'
            '<text x="124" y="68" class="lb">oxra (Fe) — qo\'ng\'ir</text>'
            '<text x="38" y="110" class="lb" font-weight="bold">pigmentlar — metall birikmalari</text></svg>')

def fig_testkit():
    """Akvarium test-to'plami: probirka + rang shkalasi."""
    return ('<svg width="220" height="118" viewBox="0 0 220 118">'
            f'<style>.lb{{font-size:8.4px;font-family:Georgia,serif;fill:{ID}}}</style>'
            f'<path d="M52,22 v56 a9,9 0 0 0 18,0 v-56" fill="none" stroke="{ID}" stroke-width="1.6"/>'
            f'<rect x="54" y="44" width="14" height="30" rx="2" fill="#7cb342" opacity="0.45"/>'
            + "".join(f'<rect x="{100+i*20}" y="40" width="18" height="18" rx="2" fill="{c}" stroke="{ID}" stroke-width="0.7"/>'
                      for i, c in enumerate(["#fff9c4", "#c5e1a5", "#7cb342", "#33691e"]))
            + '<text x="100" y="34" class="lb" font-weight="bold">rang shkalasi</text>'
            '<text x="100" y="74" class="lb">rang → ion miqdori</text>'
            '<text x="48" y="112" class="lb" font-weight="bold">suv test-to\'plami</text></svg>')

FIGS = dict(flame_panel=fig_flame_panel, precip_panel=fig_precip_panel, titr_curve=fig_titr_curve,
            bar_agx=fig_bar_agx, scheme38=fig_scheme38, iodized=fig_iodized, mineral=fig_mineral,
            paints=fig_paints, testkit=fig_testkit)
...
.gopts .go svg {{ display:block; margin: 0 auto 0.6mm; border:0.8pt solid #ecc3d3; border-radius:2pt;
                  background:#fdf2f6; padding:1mm;}}
...
letters = "ABCD"
H = [f"<meta charset='utf-8'><title>17-bob — Sifat reaksiyalari</title><style>{css}</style>"]
...
H.append(f"""
<div class="page chap">
  <div class="chapnum">17</div>
  <div class="kicker">1-kitob · Anorganik kimyo · 17-bob · Mavzu pasporti (IV.2)</div>
  <h1>Sifat reaksiyalari</h1>
  <div class="lead">kationlarni aniqlash: alanga va cho'kmalar · anionlarni aniqlash: AgNO₃, BaCl₂,
  kislota sinovlari · gazlarni tanish · tahlil ketma-ketligi</div>
  <div class="pass">
    <div class="card"><h3>Nimalarni tekshiradi</h3><ul>
      <li>ion → reagent → belgi zanjirini bilish</li>
      <li>cho'kma va alanga «rang lug'ati»</li>
      <li>cho'kma massalari hisoblari (A: 10, 12; B: 2, 15, 29)</li>
      <li>aralash eritma va detektiv masalalar (B: 11, 25, 40, 41)</li>
      <li>tahlil tartibi tuzoqlari (B: 6, 20, 25, 42)</li></ul></div>
    <div class="card"><h3>Vizual ko'nikmalar</h3><ul>
      <li>alanga ranglari paneli (A: 26, 32)</li>
      <li>cho'kmalar paneli (A: 28; B: 28)</li>
      <li>titrlash egri va AgX ustunlari (B: 5, 19, 26, 32)</li></ul></div>
    <div class="card"><h3>Tez-tez uchraydigan xatolar</h3><ul>
      <li>NO₃⁻ ga cho'kma reaksiyasi «topish»</li>
      <li>HNO₃/HCl bilan oldindan ishlovni unutish</li>
      <li>NH₃ ni kislotali gaz deb o'qish</li>
      <li>oq cho'kmalarni (AgCl/BaSO₄) farqlamaslik</li></ul></div>
  </div>
  <table class="spec">
